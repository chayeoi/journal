...
/**
 * 집필진(저자) 데이터 레이어 — E-E-A-T 저자 페이지(/authors, /authors/{id})용.
 *
 * 핵심 규칙
 * - "집필진"은 발행(is_visible)된 글을 1편 이상 가진 profiles 만 노출한다.
 * - 노출 순서: 집필 수 → 이름(가나다).
 * - profiles 의 기본 컬럼(display_name, bio, avatar_url)만 읽는다 — 별도 마이그레이션 불필요.
 */
 ...
 * 글 한 번, 프로필 한 번 — 2쿼리. 집필 수는 정렬에만 쓰고 노출하지 않는다.
 */
