...
const link = style({
  ...
  selectors: {
    ...
    // border: 행마다 헤어라인 보더 카드
    'html[data-acard="border"] &': { borderColor: "var(--line)" },
    'html[data-acard="border"] &:hover': { borderColor: "var(--ink)" },
    // shadow: 행마다 부드러운 그림자 카드
    'html[data-acard="shadow"] &': {
      boxShadow: "0 1px 2px rgba(0,0,0,0.03), 0 12px 28px -14px rgba(0,0,0,0.16)",
    },
    'html[data-acard="shadow"] &:hover': {
      boxShadow: "0 2px 4px rgba(0,0,0,0.04), 0 20px 40px -18px rgba(0,0,0,0.2)",
...
const avatarInitial = style({
  ...
  fontSize: 25,
  fontWeight: 700,
  fontFamily: "var(--font)", // sans-serif (.avatar 기본은 serif)
});
